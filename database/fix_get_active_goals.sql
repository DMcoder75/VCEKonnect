-- =====================================================
-- FIX: Update get_active_goals function to use correct table names
-- =====================================================
-- Issue: Function was querying vk_goal_periods but actual table is vk_study_goal_periods
-- Solution: Update all function references to match actual table schema
-- =====================================================

-- Drop and recreate function with corrected table names
    ...
    'weekly', (
      select json_build_object(
        ...
        'subjects', (
          ...
          from vk_study_goal_subjects gs
          where gs.goal_period_id = gp.id
          order by gs.hours_target desc
        )
      )
      from vk_study_goal_periods gp
...
begin
  ...
  update vk_study_goal_periods gp
  ...
  update vk_study_goal_subjects gs
  set minutes_achieved = (
    select coalesce(sum(ss.duration_minutes), 0)
    from vk_study_sessions ss
    inner join vk_study_goal_periods gp on gp.id = gs.goal_period_id
    where ss.user_id = gp.user_id
  ...
  where exists (
    select 1 from vk_study_goal_periods gp
...
begin
  ...
  if p_weekly is not null then
    ...
    insert into vk_study_goal_periods (
    ...
    loop
      insert into vk_study_goal_subjects (goal_period_id, subject_id, hours_target)
      values (
        v_weekly_id,
